...
#define MOD_REG_RESET                                                          \
	0x0100 // register  0 [0x0100]: reset and init register (R/W)
...
#define MOD_REG_IOCTRL                                                         \
	0x0103 // register  3 [0x0103]: input/output control (R/W)
#define MOD_REG_MOD_ADDR                                                       \
	0x0104 // register  4 [0x0104]: module i2c address (R/W, default: 0x10)
#define MOD_REG_SEN_ADDR                                                       \
	0x0105 // register  5 [0x0105]: sensor i2c address (R/W, default: 0x1A)
#define MOD_REG_OUTPUT                                                         \
	0x0106 // register  6 [0x0106]: output signal override register (R/W, default: 0x00)
#define MOD_REG_INPUT                                                          \
	0x0107 // register  7 [0x0107]: input signal status register (R)
#define MOD_REG_EXTTRIG                                                        \
	0x0108 // register  8 [0x0108]: external trigger enable (R/W, default: 0x00)
#define MOD_REG_EXPO_L                                                         \
	0x0109 // register  9 [0x0109]: exposure LSB (R/W, default: 0x10)
#define MOD_REG_EXPO_M                                                         \
	0x010A // register 10 [0x010A]: exposure 	   (R/W, default: 0x27)
#define MOD_REG_EXPO_H                                                         \
	0x010B // register 11 [0x010B]: exposure     (R/W, default: 0x00)
#define MOD_REG_EXPO_U                                                         \
	0x010C // register 12 [0x010C]: exposure MSB (R/W, default: 0x00)
#define MOD_REG_RETRIG_L                                                       \
	0x010D // register 13 [0x010D]: retrigger LSB (R/W, default: 0x40)
#define MOD_REG_RETRIG_M                                                       \
	0x010E // register 14 [0x010E]: retrigger     (R/W, default: 0x2D)
#define MOD_REG_RETRIG_H                                                       \
	0x010F // register 15 [0x010F]: retrigger     (R/W, default: 0x29)
#define MOD_REG_RETRIG_U                                                       \
	0x0110 // register 16 [0x0110]: retrigger MSB (R/W, default: 0x00)

#define REG_RESET_PWR_UP 0x00
#define REG_RESET_SENSOR                                                       \
	0x01 // reg0[0] = 0 sensor reset the sensor is held in reset when this bit is 1
#define REG_RESET_PWR_DOWN                                                     \
	0x02 // reg0[1] = 0 power down power for the sensor is switched off
#define REG_STATUS_NO_COM                                                      \
	0x00 // reg1[7:0] = 0x00 default, no communication with sensor possible
#define REG_STATUS_READY                                                       \
	0x80 // reg1[7:0] = 0x80 sensor ready after successful initialization sequence
#define REG_STATUS_ERROR                                                       \
	0x01 // reg1[7:0] = 0x01 internal error during initialization
...
__u32 vc_core_calculate_max_exposure(struct vc_cam *cam, __u8 num_lanes,
				     __u8 format, __u8 binning);
__u32 vc_core_calculate_max_frame_rate(struct vc_cam *cam, __u8 num_lanes,
				       __u8 format, __u8 binning);
static __u32 vc_core_calculate_period_1H(struct vc_cam *cam, __u8 num_lanes,
					 __u8 format, __u8 binning);
void vc_core_calculate_roi(struct vc_cam *cam, __u32 *w_left, __u32 *w_right,
			   __u32 *w_width, __u32 *w_top, __u32 *w_bottom,
			   __u32 *w_height, __u32 *o_width, __u32 *o_height);
...
static __u8 i2c_read_reg(struct device *dev, struct i2c_client *client,
			 const __u16 addr, const char *func)
{
...
static int i2c_write_reg(struct device *dev, struct i2c_client *client,
			 const __u16 addr, const __u8 value, const char *func)
{
		   ...
		   const char *func)
{
	...
	for (i = 0; regs[i].address != 0; i++) {
		i2c_write_reg(&client->dev, client, regs[i].address,
			      regs[i].value, func);
	}
...
static __u32 i2c_read_reg2(struct device *dev, struct i2c_client *client,
			   struct vc_csr2 *csr, const char *func)
{
	__u32 reg = 0;
	__u32 value = 0;
...
static int i2c_write_reg2(struct device *dev, struct i2c_client *client,
			  struct vc_csr2 *csr, const __u16 value,
			  const char *func)
{
	int ret = 0;
...
static __u32 i2c_read_reg4(struct device *dev, struct i2c_client *client,
			   struct vc_csr4 *csr, const char *func)
{
	__u32 reg = 0;
	__u32 value = 0;
...
static int i2c_write_reg4(struct device *dev, struct i2c_client *client,
			  struct vc_csr4 *csr, const __u32 value,
			  const char *func)
{
	int ret = 0;
...
int vc_write_i2c_reg(struct i2c_client *client, const __u16 addr,
		     const __u8 value)
{
...
static void vc_core_print_desc(struct device *dev, struct vc_desc *desc)
{
	...
	vc_info(dev, "| MANUF. | %s               MID: 0x%04x |\n", desc->manuf,
		desc->manuf_id);
	vc_info(dev,
		"| MODULE | ID:  0x%04x                     REV:   %04u |\n",
		desc->mod_id, desc->mod_rev);
	vc_info(dev, "| SENSOR | %s%s %s%s                                |\n",
		desc->sen_manuf,
		(0 == strcmp("OM", desc->sen_manuf)) ? "  " : "",
		desc->sen_type, is_color ? "" : " ");
	vc_info(dev,
		"+--------+---------------------------------------------+\n");
}
...
static void vc_core_print_csr(struct device *dev, struct vc_desc *desc)
{
	...
	vc_info(dev,
		...
		desc->csr_mode);
	vc_info(dev,
		"| horizontal start          | 0x%04x | 0x%04x |        |\n",
		desc->csr_h_start_l, desc->csr_h_start_h);
	vc_info(dev,
		"| vertical start            | 0x%04x | 0x%04x |        |\n",
		desc->csr_v_start_l, desc->csr_v_start_h);
	vc_info(dev,
		"| horizontal end            | 0x%04x | 0x%04x |        |\n",
		desc->csr_h_end_l, desc->csr_h_end_h);
	vc_info(dev,
		"| vertical end              | 0x%04x | 0x%04x |        |\n",
		desc->csr_v_end_l, desc->csr_v_end_h);
	vc_info(dev,
		"| hor. output width         | 0x%04x | 0x%04x |        |\n",
		desc->csr_o_width_l, desc->csr_o_width_h);
	vc_info(dev,
		"| ver. output height        | 0x%04x | 0x%04x |        |\n",
		desc->csr_o_height_l, desc->csr_o_height_h);
	vc_info(dev,
		"| exposure                  | 0x%04x | 0x%04x | 0x%04x |\n",
		desc->csr_exposure_l, desc->csr_exposure_m,
		desc->csr_exposure_h);
	vc_info(dev,
		"| gain                      | 0x%04x | 0x%04x |        |\n",
...
static void vc_core_print_modes(struct device *dev, struct vc_desc *desc)
{
	...
	vc_info(dev,
		"+--- Module Modes -------+---------+---------+---------+\n");
	vc_info(dev,
		"|  # | rate    | lanes   | format  | type    | binning |\n");
	vc_info(dev,
		"+----+---------+---------+---------+---------+---------+\n");
	for (index = 0; index < desc->num_modes; index++) {
		...
		switch (mode->type) {
		case MODE_TYPE_STREAM:
			strcpy(type, "STREAM ");
			break;
		case MODE_TYPE_TRIGGER:
			strcpy(type, "EXT.TRG");
			break;
		case MODE_TYPE_SLAVE:
			strcpy(type, "SLAVE  ");
			break;
		default:
			sprintf(type, "0x%02x   ", mode->type);
			break;
		}
		vc_info(dev,
			"| %2d |    %4u |       %u | %s   | %s |       %u |\n",
			index, data_rate, mode->num_lanes, format, type,
			mode->binning);
	}
	vc_info(dev,
		"+----+---------+---------+---------+---------+---------+\n");
...
__u8 vc_core_mbus_to_bpp(__u32 code)
{
        switch (code) {
        case MEDIA_BUS_FMT_Y8_1X8:
        case MEDIA_BUS_FMT_SRGGB8_1X8:
...
EXPORT_SYMBOL(vc_core_mbus_to_bpp);
...
static __u32 vc_core_format_to_mbus_code(__u8 format, int is_color, int is_gbrg)
{
	switch (format) {
	case FORMAT_RAW08:
		return is_color ? (is_gbrg ? MEDIA_BUS_FMT_SGBRG8_1X8 :
					     MEDIA_BUS_FMT_SRGGB8_1X8) :
				  MEDIA_BUS_FMT_Y8_1X8;
	case FORMAT_RAW10:
		return is_color ? (is_gbrg ? MEDIA_BUS_FMT_SGBRG10_1X10 :
					     MEDIA_BUS_FMT_SRGGB10_1X10) :
				  MEDIA_BUS_FMT_Y10_1X10;
	case FORMAT_RAW12:
		return is_color ? (is_gbrg ? MEDIA_BUS_FMT_SGBRG12_1X12 :
					     MEDIA_BUS_FMT_SRGGB12_1X12) :
				  MEDIA_BUS_FMT_Y12_1X12;
	case FORMAT_RAW14:
		return is_color ? (is_gbrg ? MEDIA_BUS_FMT_SGBRG14_1X14 :
...
vc_mode vc_core_get_mode_by_param(struct vc_cam *cam, __u8 num_lanes,
				  __u8 format, __u8 binning)
{
...
vc_mode vc_core_get_mode(struct vc_cam *cam)
{
	...
	return vc_core_get_mode_by_param(cam, state->num_lanes, format,
					 binning);
}
EXPORT_SYMBOL(vc_core_get_mode);

__u32 vc_core_get_hmax(struct vc_cam *cam, __u8 num_lanes, __u8 format,
		       __u8 binning)
{
...
vc_control vc_core_get_blacklevel(struct vc_cam *cam, __u8 num_lanes,
				  __u8 format, __u8 binning)
{
	return vc_core_get_mode_by_param(cam, num_lanes, format, binning)
		.blacklevel;
}
...
int vc_core_set_format(struct vc_cam *cam, __u32 code)
{
	...
	if (vc_core_try_format(cam, code)) {
		...
		vc_err(dev,
		       "%s(): Format 0x%04x not supported! (Set default format: 0x%04x)\n",
...
static void vc_core_limit_frame_position(struct vc_cam *cam, __u32 left,
					 __u32 top)
{
	struct vc_ctrl *ctrl = &cam->ctrl;
	struct vc_state *state = &cam->state;
...
static void vc_core_limit_frame_size(struct vc_cam *cam, __u32 width,
				     __u32 height)
{
	struct vc_ctrl *ctrl = &cam->ctrl;
	struct vc_state *state = &cam->state;
...
int vc_core_set_frame(struct vc_cam *cam, __u32 left, __u32 top, __u32 width,
		      __u32 height)
{
	struct vc_state *state = &cam->state;
	struct device *dev = vc_core_get_sen_device(cam);
	...
	vc_info(dev,
		"%s(): Set frame (left: %u, top: %u, width: %u, height: %u)\n",
	...
	if (state->frame.left != left || state->frame.top != top ||
	    state->frame.width != width || state->frame.height != height) {
		vc_warn(dev,
			"%s(): Adjusted frame (left: %u, top: %u, width: %u, height: %u)\n",
			__FUNCTION__, state->frame.left, state->frame.top,
...
int vc_core_set_link_frequencies(struct vc_cam *cam, __u64 *link_frequencies, __u32 num_link_frequencies)
{
        struct vc_state *state = &cam->state;

        state->link_frequencies = link_frequencies;
        state->num_link_frequencies = num_link_frequencies;
        return 0;
}
EXPORT_SYMBOL(vc_core_set_link_frequencies);
...
__u32 vc_core_get_optimized_vmax(struct vc_cam *cam)
{
	...
	__u32 vmax_def =
		vc_core_get_vmax(cam, num_lanes, format, binning_mode).def;
				       ...
				       __u8 format, __u8 binning)
{
	struct device *dev = vc_core_get_sen_device(cam);
	__u32 period_1H_ns =
		vc_core_calculate_period_1H(cam, num_lanes, format, binning);
...
struct i2c_client *vc_mod_get_client(struct device *dev,
				     struct i2c_adapter *adapter, __u8 i2c_addr)
{
...
int vc_mod_set_power(struct vc_cam *cam, int on)
{
	...
	ret = i2c_write_reg(dev, client_mod, MOD_REG_RESET,
			    on ? REG_RESET_PWR_UP : REG_RESET_PWR_DOWN,
			    __FUNCTION__);
	if (ret) {
		vc_err(dev, "%s(): Unable to power %s the module (error: %d)\n",
		       __FUNCTION__, (on == REG_RESET_PWR_UP) ? "up" : "down",
		       ret);
...
static int vc_mod_write_trigger_mode(struct i2c_client *client, int mode)
{
	...
	if (ret)
		vc_err(dev,
		       "%s(): Unable to write external trigger (error: %d)\n",
		       __FUNCTION__, ret);
...
static int vc_mod_setup(struct vc_ctrl *ctrl, int mod_i2c_addr,
			struct vc_desc *desc)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		vc_err(dev_sen,
		       "%s(): I2C-Adapter doesn't support I2C_FUNC_SMBUS_BYTE\n",
		       __FUNCTION__);
		return 0;
	}
	...
	if (client_mod == 0) {
		vc_err(dev_sen,
		       "%s(): Unable to get module I2C client for address 0x%02x\n",
	...
	for (addr = 0; addr < sizeof(*desc); addr++) {
		reg = i2c_read_reg(dev_mod, client_mod, addr + 0x1000,
				   __FUNCTION__);
		if (reg < 0) {
	...
	if (desc->num_modes == 0) {
		vc_err(dev_mod,
		       "%s(): Could not find any module modes! Operation not possible!\n",
		       __FUNCTION__);
		return -EIO;
	}
...
int vc_core_update_controls(struct vc_cam *cam)
{
	...
	__u8 binning = state->binning_mode;
	if (ctrl->flags & FLAG_INCREASE_FRAME_RATE) {
		ctrl->exposure.max = vc_core_calculate_max_exposure(
			cam, num_lanes, format, binning);
...
static void vc_core_state_init(struct vc_cam *cam)
{
	...
	blacklevel_def =
		vc_core_get_blacklevel(cam, state->num_lanes, format, binning)
			.def;
	blacklevel_max =
		vc_core_get_blacklevel(cam, state->num_lanes, format, binning)
			.max +
		1;
	state->blacklevel = (__u32)DIV_ROUND_CLOSEST(blacklevel_def * 100000,
						     blacklevel_max);
...
int vc_core_init(struct vc_cam *cam, struct i2c_client *client)
{
	...
	vc_info(&ctrl->client_mod->dev,
		"VC MIPI Core successfully initialized");
	return 0;
}
...
static int vc_mod_write_exposure(struct i2c_client *client, __u32 value)
{
	...
	ret = i2c_write_reg(dev, client, MOD_REG_EXPO_L, L_BYTE(value),
			    __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_EXPO_M, M_BYTE(value),
			     __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_EXPO_H, H_BYTE(value),
			     __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_EXPO_U, U_BYTE(value),
			     __FUNCTION__);
...
static int vc_mod_write_retrigger(struct i2c_client *client, __u32 value)
{
	...
	ret = i2c_write_reg(dev, client, MOD_REG_RETRIG_L, L_BYTE(value),
			    __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_RETRIG_M, M_BYTE(value),
			     __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_RETRIG_H, H_BYTE(value),
			     __FUNCTION__);
	ret |= i2c_write_reg(dev, client, MOD_REG_RETRIG_U, U_BYTE(value),
			     __FUNCTION__);

	return ret;
}

static __u8 vc_mod_find_mode(struct vc_cam *cam, __u8 num_lanes, __u8 format,
			     __u8 type, __u8 binning)
{
	struct vc_desc *desc = &cam->desc;
	struct device *dev = vc_core_get_mod_device(cam);
	...
	for (index = 0; index < desc->num_modes; index++) {
		...
		if (mode->num_lanes == num_lanes && mode->format == format &&
		    mode->type == type && mode->binning == binning &&
...
static int vc_mod_write_mode(struct i2c_client *client, __u8 mode)
{
	...
	if (ret)
		vc_err(dev,
		       "%s(): Unable to write module mode: 0x%02x (error: %d)\n",
...
static __u32 vc_sen_read_hmax(struct vc_ctrl *ctrl)
{
	...
	struct device *dev = &client->dev;
	__u32 hmax =
		i2c_read_reg4(dev, client, &ctrl->csr.sen.hmax, __FUNCTION__);
...
static __u32 vc_sen_read_vmax(struct vc_ctrl *ctrl)
{
	...
	struct device *dev = &client->dev;
	__u32 vmax =
		i2c_read_reg4(dev, client, &ctrl->csr.sen.vmax, __FUNCTION__);
...
int vc_mod_set_mode(struct vc_cam *cam, int *reset)
{
	...
	vc_info(dev,
		"%s(): Set module mode: %u (lanes: %u, format: %s, type: %s)\n",
		__FUNCTION__, mode, num_lanes, fourcc, stype);

	ret = vc_mod_reset_module(cam, mode);
	if (ret) {
		vc_err(dev,
		       "%s(): Unable to set module mode: %u (lanes: %u, format: %s, type: %s) (error: %d)\n",
		       __func__, mode, num_lanes, fourcc, stype, ret);
		return ret;
	}
...
int vc_mod_set_trigger_mode(struct vc_cam *cam, int mode)
{
	...
	if (mode == 0) {
		...
		state->trigger_mode = REG_TRIGGER_PULSEWIDTH;
		   ...
		   ctrl->flags & (FLAG_TRIGGER_SELF | FLAG_TRIGGER_SELF_V2)) {
		mode_desc = "SELF";
		...
		state->trigger_mode = REG_TRIGGER_SINGLE;
		   ...
		   ctrl->flags & (FLAG_TRIGGER_SYNC | FLAG_TRIGGER_SLAVE)) {
		mode_desc = "SYNC";
...
int vc_mod_set_single_trigger(struct vc_cam *cam)
{
	...
	return i2c_write_reg(dev, client, MOD_REG_EXTTRIG, REG_TRIGGER_SINGLE,
			     __FUNCTION__);
}
...
int vc_mod_set_io_mode(struct vc_cam *cam, int mode)
{
	...
	if (mode == 0) {
		...
		case 2:
			...
			state->io_mode =
				REG_IO_FLASH_ENABLE | REG_IO_FLASH_ACTIVE_LOW;
			break;
		case 3:
		...
		case 4:
			...
			state->io_mode =
				REG_IO_FLASH_ENABLE | REG_IO_TRIG_ACTIVE_LOW;
			break;
		case 5:
			mode_desc = "TRIGGER AND FLASH ACTIVE LOW";
			state->io_mode = REG_IO_FLASH_ENABLE |
					 REG_IO_FLASH_ACTIVE_LOW |
					 REG_IO_TRIG_ACTIVE_LOW;
			break;
		}
...
int vc_sen_write_mode(struct vc_ctrl *ctrl, int mode)
{
	...
	if (mode == ctrl->csr.sen.mode_standby) {
		...
		if (ctrl->csr.sen.mode.l) {
			ret = i2c_write_reg(dev, client, ctrl->csr.sen.mode.l,
					    value, __FUNCTION__);
		}
		if (ctrl->csr.sen.mode.m) {
			ret |= i2c_write_reg(dev, client, ctrl->csr.sen.mode.m,
					     value, __FUNCTION__);
		}
	} else {
	...
	if (ret)
		vc_err(dev,
		       "%s(): Couldn't write sensor mode: 0x%02x (error: %d)\n",
		       __FUNCTION__, mode, ret);
...
void vc_core_calculate_roi(struct vc_cam *cam, __u32 *left, __u32 *right,
			   __u32 *width, __u32 *top, __u32 *bottom,
			   __u32 *height, __u32 *o_width, __u32 *o_height)
{
	struct vc_ctrl *ctrl = &cam->ctrl;
	struct vc_state *state = &cam->state;
	struct i2c_client *client = ctrl->client_sen;
...
int vc_sen_write_binning_mode_regs(struct vc_cam *cam)
{
	...
	while (regs[index].address > 0) {
		ret |= i2c_write_reg(dev, client, regs[index].address,
				     regs[index].value, __FUNCTION__);
		index++;
	}
...
static int vc_sen_write_hmax(struct vc_ctrl *ctrl, __u32 hmax)
{
	...
	return i2c_write_reg4(dev, client, &ctrl->csr.sen.hmax, hmax,
			      __FUNCTION__);
}
...
int vc_sen_set_roi(struct vc_cam *cam)
{
	...
	struct device *dev = &client->dev;
	struct vc_binning *binning = vc_core_get_binning(cam);
	int w_left, w_top, w_right, w_bottom, w_width, w_height, o_width,
		o_height;
	int ret = 0;
	...
	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.o_width, o_width,
			      __FUNCTION__);
	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.o_height, o_height,
			      __FUNCTION__);
	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.h_start, w_left,
			      __FUNCTION__);
	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.v_start, w_top,
			      __FUNCTION__);

	if (ctrl->flags & FLAG_PREGIUS_S) {
		ret |= i2c_write_reg2(dev, client, &vc2OP_BLK_HWIDTH, o_width,
				      __FUNCTION__);
		ret |= i2c_write_reg2(dev, client, &vc2INFO_HWIDTH, o_width,
				      __FUNCTION__);
	} else {
		if (MOD_ID_IMX412 == desc->mod_id) {
			ret |= i2c_write_reg2(dev, client,
					      &DIG_CROP_IMAGE_WIDTH, o_width,
					      __FUNCTION__);
			ret |= i2c_write_reg2(dev, client,
					      &DIG_CROP_IMAGE_HEIGHT, o_height,
					      __FUNCTION__);
		}
		ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.h_end,
				      w_right, __FUNCTION__);
		ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.v_end,
				      w_bottom, __FUNCTION__);
		ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.w_width,
				      w_width, __FUNCTION__);
		ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.w_height,
				      w_height, __FUNCTION__);
	}
	...
	if (ret) {
		vc_err(dev,
		       "%s(): Couldn't set sensor roi: "
		       "(left-width-right: %u+%u=%u=>%u, top-height-bottom: %u+%u=%u=>%u)\n",
		       __FUNCTION__, w_left, w_width, w_right, o_width, w_top,
		       w_height, w_bottom, o_height);
		return ret;
	}
...
static int vc_core_mdB_to_times(int mdB)
{
	...
	for (index = 1; index <= mdB; index++) {
		times = div_u64(times * base, 100000000);
	}
	return div_u64(times, 100000);
}
...
int vc_sen_set_gain(struct vc_cam *cam, int gain_mdB)
{
	...
	case GAIN_LINEAR:
		gain = div_u64(((__u64)ctrl->gain.max) * gain_mdB,
			       ctrl->gain.max_mdB);
		break;
	...
	vc_info(dev,
		"%s(): Set sensor gain: %u mdB (%u/%u) (exposure: %u us)\n",
		__FUNCTION__, gain_mdB, gain, ctrl->gain.max,
		cam->state.exposure);

	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.gain, gain,
			      __FUNCTION__);
	if (ret) {
...
int vc_sen_set_blacklevel(struct vc_cam *cam, __u32 blacklevel_rel)
{
	...
	__u8 binning = state->binning_mode;
	__u32 blacklevel_max =
		vc_core_get_blacklevel(cam, num_lanes, format, binning).max;
	__u32 blacklevel_abs = (__u32)DIV_ROUND_CLOSEST(
		(blacklevel_rel * blacklevel_max), 100000);

	vc_info(dev, "%s(): Set sensor black level: %u (%u/%u)\n", __FUNCTION__,
		blacklevel_rel, blacklevel_abs, blacklevel_max);

	ret |= i2c_write_reg2(dev, client, &ctrl->csr.sen.blacklevel,
			      blacklevel_abs, __FUNCTION__);
	if (ret) {
...
int vc_sen_start_stream(struct vc_cam *cam)
{
	...
	if ((ctrl->flags & FLAG_EXPOSURE_SONY ||
	    ...
	    (ctrl->flags & FLAG_EXPOSURE_OMNIVISION &&
	     !vc_mod_is_trigger_enabled(cam))) {
		ret |= vc_sen_write_mode(ctrl, ctrl->csr.sen.mode_operating);
		if (ret)
			vc_err(dev,
			       "%s(): Unable to start streaming (error: %d)\n",
			       __FUNCTION__, ret);
	}

	if (ctrl->flags & FLAG_TRIGGER_SLAVE &&
	    state->trigger_mode == REG_TRIGGER_SYNC) {
		ret |= vc_mod_write_io_mode(client_mod, REG_IO_XTRIG_ENABLE);
		ret |= vc_mod_write_trigger_mode(client_mod,
						 REG_TRIGGER_DISABLE);
					 ...
					 __u8 format, __u8 binning)
{
	...
	for (index = 0; index <= MAX_VC_MODES; index++) {
		...
		if (mode->num_lanes == num_lanes && mode->format == format &&
		    (binning_index == ctrl->mode[index].binning)) {
			return div_u64((__u64)mode->hmax * 1000000000,
				       ctrl->clk_pixel);
		}
	}
...
__u32 vc_core_get_time_per_line_ns(struct vc_cam *cam)
{
	struct vc_state *state = &cam->state;
	__u8 format = vc_core_mbus_code_to_format(state->format_code);
	return vc_core_calculate_period_1H(cam, state->num_lanes, format,
					   state->binning_mode);
}
...
int vc_core_set_binning_mode(struct vc_cam *cam, int mode)
{
	...
	if (mode > ctrl->max_binning_modes_used) {
		vc_err(dev,
		       "%s(): Couldn't set binning mode (max supported modes: %d)\n",
...
static void vc_calculate_exposure(struct vc_cam *cam, __u32 exposure_us)
{
	...
	exposure_1H = div_u64(exposure_ns, period_1H_ns);
...
static void vc_calculate_trig_exposure(struct vc_cam *cam, __u32 exposure_us)
{
	...
	__u32 retrigger_min =
		vc_core_get_retrigger(cam, num_lanes, format, binning);
	...
	min_frametime_us =
		div_u64((__u64)retrigger_min * 1000000, ctrl->clk_ext_trigger);
	frametime_us = min_frametime_us;

	if (state->trigger_mode & REG_TRIGGER_SELF) {
		if (state->framerate > 0) {
			frametime_us = div_u64(1000000000, state->framerate);
		}
	...
	state->retrigger_cnt =
		div_u64((__u64)frametime_us * ctrl->clk_ext_trigger, 1000000);
	// NOTE: Check this for different cameras.
	// if (state->retrigger_cnt < 3240) {
	// 	state->retrigger_cnt = 3240;
	// }
	state->exposure_cnt =
		div_u64((__u64)exposure_us * ctrl->clk_ext_trigger, 1000000);
}
...
int vc_sen_set_exposure(struct vc_cam *cam, int exposure_us)
{
	...
	if (ctrl->flags & FLAG_EXPOSURE_SONY ||
		...
		case REG_TRIGGER_SELF:
			...
			ret |= vc_mod_write_exposure(client_mod,
						     state->exposure_cnt);
			...
			if (!state->streaming ||
			    ctrl->flags & FLAG_TRIGGER_SELF_V2) {
				ret |= vc_mod_write_retrigger(
					client_mod, state->retrigger_cnt);
			}
			break;
		case REG_TRIGGER_PULSEWIDTH:
			break;
		...
		__u32 duration =
			div_u64(((__u64)exposure_us) * ctrl->flash_factor, 1000000);
